-- Migration: create_revenue_events
-- Created at: 1762112278
...
CREATE INDEX IF NOT EXISTS idx_revenue_events_event_timestamp ON revenue_events(event_timestamp);
CREATE INDEX IF NOT EXISTS idx_revenue_events_job_id ON revenue_events(job_id);
...
DROP TRIGGER IF EXISTS revenue_events_updated_at ON revenue_events;
...
-- Create policies
DROP POLICY IF EXISTS "Admin users can manage revenue events" ON revenue_events;
...
DROP POLICY IF EXISTS "Service role can insert revenue events" ON revenue_events;
