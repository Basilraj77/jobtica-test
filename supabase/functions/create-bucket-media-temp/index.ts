...
Deno.serve(async (req) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Upload for media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'media');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for media" ON storage.objects FOR UPDATE USING (bucket_id = 'media');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for media" ON storage.objects FOR DELETE USING (bucket_id = 'media');`
    ];
